YUI.add('moodle-mod_scheduler-mydateselector', function(Y) {
    ...
    CALENDAR.prototype = {
        ...
        focus_event : function(e) {
            M.mod_scheduler.mydateselector.cancel_any_timeout();
            // If the current owner is set, then the pop-up is currently being displayed, so hide it.
            if (M.mod_scheduler.mydateselector.currentowner == this) {
        ...
        claim_calendar : function() {
            ...
            if (M.mod_scheduler.mydateselector.currentowner) {
                M.mod_scheduler.mydateselector.currentowner.release_calendar();
            }
            if (M.mod_scheduler.mydateselector.currentowner != this) {
                this.connect_handlers();
                this.set_date_from_selects();
            }
            M.mod_scheduler.mydateselector.currentowner = this;
            M.mod_scheduler.mydateselector.calendar.cfg.setProperty('mindate', new Date(this.yearselect.firstOptionValue(), 0, 1));
            M.mod_scheduler.mydateselector.calendar.cfg.setProperty('maxdate', new Date(this.yearselect.lastOptionValue(), 11, 31));
            M.mod_scheduler.mydateselector.panel.show();
            M.mod_scheduler.mydateselector.fix_position();
            setTimeout(function(){M.mod_scheduler.mydateselector.cancel_any_timeout()}, 100);
        },
        set_date_from_selects : function() {
            ...
            M.mod_scheduler.mydateselector.calendar.select(date);
            M.mod_scheduler.mydateselector.calendar.setMonth(month);
            M.mod_scheduler.mydateselector.calendar.setYear(year);
            M.mod_scheduler.mydateselector.calendar.render();
            if (date.getDate() != day) {
        ...
        set_selects_from_date : function(eventtype, args) {
            ...
            if (M.mod_scheduler.dateselector.currentowner && this.closepopup) {
                this.release_calendar();
            }
        },
        connect_handlers : function() {
            M.mod_scheduler.mydateselector.calendar.selectEvent.subscribe(this.set_selects_from_date, this, true);
        },
        release_calendar : function() {
            M.mod_scheduler.mydateselector.panel.hide();
            M.mod_scheduler.mydateselector.currentowner = null;
            M.mod_scheduler.mydateselector.calendar.selectEvent.unsubscribe(this.set_selects_from_date, this);
        },
    ...
    M.mod_scheduler.mydateselector = {
        panel : null,
        calendar : null,
        currentowner : null,
        hidetimeout : null,
        ...
        initPanel : function(config) {
            this.panel = new Y.Overlay({
                visible : true,  //tdmu
                bodyContent : Y.Node.create('<div id="mydateselector-calendar-content">test</div>'),
                id : 'mydateselector-calendar-panel'
            });
            ...
            Y.one('#mydateselector-calendar-panel').setStyle('zIndex', null);
            this.panel.on('heightChange', this.fix_position, this);

            Y.one('#mydateselector-calendar-panel').on('click', function(e){e.halt();});
            Y.one(document.body).on('click', this.document_click, this);

            this.calendar = new Y.YUI2.widget.Calendar(document.getElementById('mydateselector-calendar-content'), {
                iframe: false,
